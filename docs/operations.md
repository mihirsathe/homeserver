...
4. `python3 scripts/bootstrap.py` — reconnects Prowlarr/Overseerr/Bazarr/Tautulli with the new key.

### Cloudflare tunnel token
1. Cloudflare Zero Trust → Networks → Tunnels → select tunnel → rotate token.
2. Edit `.env`: update `CLOUDFLARE_TUNNEL_TOKEN`.
3. `docker compose up -d cloudflared` — the container picks up the new token on restart.
