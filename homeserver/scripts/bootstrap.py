...
def main() -> None:
    ...
    print()
    print("  Unraid GUI — NOT dependent on Caddy or Docker, works with the array")
    print("  stopped. This is the emergency path; use it if *.lan is down:")
    print(f"    http://{lan_ip}/            (LAN)")
    tailnet_host = get(env, "TAILNET_HOST_IP")
    if tailnet_host:
        print(f"    http://{tailnet_host}/         (over Tailscale)")
    print("    http://unraid.lan        (convenience alias via Caddy — not the")
    print("                              guaranteed path; it dies with Caddy)")
    print()
    ...
    print("  3. Profilarr → http://profilarr.lan → add Radarr + Sonarr as sync")
