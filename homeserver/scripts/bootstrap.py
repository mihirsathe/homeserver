...
def main() -> None:
    ...
    # Caddy publishes on the host's CADDY_HTTP_PORT (81 by default — :80 is
    # Unraid's web GUI), so every admin URL carries the port.
    cp = get(env, "CADDY_HTTP_PORT", "81")
    ...
    print()
    print(f"  SABnzbd:   http://sab.lan:{cp}")
    print(f"  Prowlarr:  http://prowlarr.lan:{cp}")
    print(f"  Radarr:    http://radarr.lan:{cp}")
    print(f"  Sonarr:    http://sonarr.lan:{cp}")
    print(f"  Lidarr:    http://lidarr.lan:{cp}")
    print(f"  Bazarr:    http://bazarr.lan:{cp}")
    print(f"  Tautulli:  http://tautulli.lan:{cp}")
    print(f"  Profilarr: http://profilarr.lan:{cp}")
    print(f"  Seerr:     http://seerr.lan:{cp}")
    print(f"  AdGuard:   http://adguard.lan:{cp}   (admin pass in generated.env)")
    print(f"  Plex:      http://{lan_ip}:32400/web   (public — direct, not via Caddy)")
    print(f"  Unraid:    http://{lan_ip}/   (unchanged — Caddy deliberately avoids :80)")
    print()
    ...
    print(f"  3. Profilarr → http://profilarr.lan:{cp} → add Radarr + Sonarr as sync")
