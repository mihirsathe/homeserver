...
# Resolve the directory this script lives in so we can invoke sibling
# scripts (e.g. seed-share-structure.sh) regardless of the caller's cwd.
...
if [[ ! -d /mnt/user ]]; then
    ...
    # Pre-create the same category skeleton on every array disk so Unraid's
    # per-path allocator has an existing target on each disk before any
    # real writes happen. Without this, the first bulk seed of the media
    # library pins every category path to whichever disk the allocator
    # picked first (usually disk1), and all future writes for that path
    # stay pinned there — see docs/decisions.md § "Unraid allocator per-
    # path pinning" and scripts/seed-share-structure.sh for the details.
    "$SCRIPT_DIR/seed-share-structure.sh"
fi
...
# Re-seed the category skeleton on every array disk before any container
# starts writing. This is idempotent for existing disks and picks up any
# disk that has been added to the array since the last boot — without it,
# the Unraid allocator would pin every new write for a category to
# whichever disk already contained that category's folder. See
# docs/decisions.md § "Unraid allocator per-path pinning".
bash "$STACK_DIR/scripts/seed-share-structure.sh"
cd "$STACK_DIR"
