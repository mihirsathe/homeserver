...
# No Caddyfile pre-flight any more — there is no Caddyfile. Admin ingress is
# the host's tailscaled advertising a Tailscale Service per UI, which is
# outside this script's lifecycle entirely: a bad compose file can no longer
# take down the way in.
