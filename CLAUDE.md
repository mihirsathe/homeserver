...
| [docs/audit-2026-09-08.md](docs/audit-2026-09-08.md) | Full audit findings: backups, access control, script bugs, docs drift |
...
- **Nextcloud's data directory is `/mnt/user/nextcloud/data`, a subdirectory — never the share root** — `emhttpd` runs `chmod 0777` + `chown nobody:users` on every share root at every array start, Nextcloud refuses to serve (503) a world-readable data dir, and `www-data` cannot chmod a directory it doesn't own. It took the service down on the first reboot after deploy (2026-09-08). The subdir stays `33:33` / `0770`; `verify-stack.sh` asserts exactly that and that nothing is stranded on `/mnt/cache/nextcloud` (the share is cache=no, so the mover would never rescue it).
- **Local-AI consumers join the `ai` network and use `http://ollama:11434`** — access is opt-in — a container has to join it explicitly. Today the members besides Ollama are `actual-ai`, `coach`, and `claudecoach` (the fitness coach, deployed from its own compose in `/mnt/user/appdata/claudecoach/repo`; it joins by the network's fixed name). Ollama has no auth, so network membership is the access control. The network is declared `name: ai` (not the Compose-prefixed default) so Unraid-template containers can join it from the Docker tab. Ollama is deliberately advertised as no Tailscale Service, so nothing on the tailnet can reach it — it has no authentication, and reaching `:11434` means being able to delete every model on the box.
...
| Nextcloud user files | `/mnt/user/nextcloud/data/` — own share, array-direct, SMB off; the data dir is a **subdirectory** (see Key Conventions). **NOT covered by Appdata Backup**; `BACKUP_NEXTCLOUD_REMOTE` is the only copy |
